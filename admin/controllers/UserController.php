...
class UserController extends BaseController
{
    ...
    public function actionMyWxLogin($code) //获取openid，自定义登录态
    {
        $data=array();
        $json=$_REQUEST; //小程序请求
        $appid = Basefun::model()->get_appid();  //appId
    ...
    //根据用户id获取用户信息
    public function actionGetUserInfoById($id)
    {
        // $data = array();
        // $user = User::model()->find('account='.$id);
        // echo CJSON::encode($user);
        $s = 'name,school,grade,class,phone,signature,fans_num,collection_num';
        $criteria = new CDbCriteria;
        $criteria->condition = ('account='.$id);

        $tmp = User::model()->find($criteria);
        $data = BaseLib::model()->oneToArray($tmp,$s);
        if($data){
            Basefun::model()->echoEncode(array('data' => $data));
        }
    }
    ...
    public function actionGetUserInfo()
    {
        ...
        $tmp = User::model()->find($criteria);
        if(empty($tmp)){
            echo CJSON::encode("该用户未注册");
        }
        else{
            $s1 = 'userid,account,password,name,school,grade,class,phone,signature,wx_openid,fans_num';
            $s1 .= 'collection_num,id';
            $data = BaseLib::model()->oneToArray($tmp, $s1);
            if($data){
                Basefun::model()->echoEncode(array('data' => $data));
            }
        }
        ...
        //             $s1 .= ',province,city,sex,sexid,registerdate,idnum,parents,p_phone,class,aller';
        //             break;
        //         case "老师":
        //             $s1 = 'id,openid,unionid,header,name,salary,phone,sexid,sex,birthday,province,city,';
        //             $s1 .= 'idnum,type,resume';
        //             break;
        //         case "家长":
        //             $s1 = 'id,openid,unionid,header,name,nikename,phone,sex_id,sex,province,city,idnum,kids';
    ...
    //上传用户信息
    public function actionUpdateUserInfo($user)
    {
        $data = array();
        $found_user = User::model()->find("wx_openid='".$user->open_id."'");
        $msg = 'false';
        if(isset($found_user))
        {
            $found_user->name = $user->name;
            $found_user->grade = $user->grade;
            $found_user->signature = $user->signature;
            $s = $found_user->save();
            if($s) $msg = 'succsee';
        }
        else put_msg("user not found");
        $data['msg'] = $msg;
        echo CJSON::encode($data);
    }
    ...
    //根据open_id获取用户投稿列表
    //article->author_id ==? user->account
    public function actionGetUserArticleCollection($open_id)
    {
        $s = 'title,column_id,column_name,status_name,submit_time';
        $user = User::model()->find("wx_openid='".$open_id."'");
        $criteria = new CDbCriteria;
        $criteria->condition = ('author_id='.$user->account.' order by submit_time DESC');
    ...
    public function actiongetArticleCollection()
    {
        ...
        $json=$_REQUEST;
        $msg="not_found";
        $user=User::model()->find("wx_openid='"."1"."'");
        $article=array();
        if(isset($user))
        {
